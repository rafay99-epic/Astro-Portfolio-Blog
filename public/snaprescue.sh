...
check $? "Fetching the latest release URL"

if [ -z "$LATEST_RELEASE_URL" ]; then
    check 1 "Could not fetch the latest release or asset not found."
fi
...
temp_file=$(mktemp)
curl -fsL "$LATEST_RELEASE_URL" -o "$temp_file"
check $? "Downloading the asset file"

# Extract the downloaded .tar.xz archive
echo "Extracting the file..."
mkdir -p $DOWNLOAD_DIR
tar -xf "$temp_file" -C $DOWNLOAD_DIR
check $? "Extracting the file"

# Navigate to the extracted folder
cd $DOWNLOAD_DIR || check 1 "Could not navigate to extraction directory"
...
if [ -f "$SCRIPT_NAME" ]; then
    ...
    chmod +x $SCRIPT_NAME || check 1 "Failed to make $SCRIPT_NAME executable."

    # Run the script directly without a new shell
    $SCRIPT_NAME
    check $? "Executing $SCRIPT_NAME"
    ...
    cd ..
    rm -f "$temp_file"  # Remove the tar.xz file
    rm -rf "$DOWNLOAD_DIR"  # Remove the extraction directory
    printf '%sClean-up completed.%s\n' "$green" "$rc"
else
    check 1 "No $SCRIPT_NAME found. Make sure the correct script is present in the archive."
fi
