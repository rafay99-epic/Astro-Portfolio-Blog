...
Building this small application was cool and I learned like a lot, I didn't now image process can be thing much work, Most of my times was done making sure that image compress was working becasue in the begining it was not working very well, designing the UI was simple task.

If you like to try it out then visit the website [Image Compser](https://imagecomposer-production.up.railway.app/), Or want to checkout the code then all the code is hosted on [github](https://github.com/rafay99-epic/ImageComposer) as well.

So I tried in production, I compress all the big images in to small images adn changed some files in the code base, removed some JavaScript from the Astro Blog Site, Performace went up like from 50% to 100%, You don;t believe me check the Speed
