...
// ... (Playwright trying to get its house in order) ...
...
Here’s a simplified look at the core of my `MermaidRenderer` component:

```javascript
// src/components/ReactComponent/blog/enhancements/diagram-renderer/MermaidRenderer.tsx
import React, { useEffect, useState, useCallback, memo } from "react";
// ... (Your memoized icon and button components)
...
const MermaidRenderer = memo(function MermaidRenderer() {
  ...
  // Utility to determine diagram type (flowchart, sequence, etc.) for better UX
  const extractDiagramType = useCallback((mermaidCode: string): string => { /* ... */ }, []);

  // Function to copy the rendered SVG to clipboard
  const copyToClipboard = useCallback(async (svgElement: SVGElement, diagramId: string) => { /* ... */ }, []);

  // Initialize Mermaid library only once (and lazy load it!)
  useEffect(() => {
    const initMermaid = async () => {
      try {
        const mermaid = (await import("mermaid")).default; // This is key for performance!
        mermaid.initialize({
          theme: "base",
          themeVariables: { /* ... my Tokyo Night theme config ... */ },
          startOnLoad: false, // We'll manually render
          flowchart: { htmlLabels: true, curve: "basis" },
        });
        setMermaidInitialized(true);
      } catch (error) { console.error("Failed to initialize Mermaid:", error); }
    };
    initMermaid();
  }, []);

  // The core rendering logic: Watches for new Mermaid code blocks and renders them
  useEffect(() => {
    ...
    const renderDiagrams = async () => {
      ...
      for (let i = 0; i < mermaidBlocks.length; i++) {
        ...
        // ... logic to create a new container for the diagram ...
        ...
        svgWrapper.innerHTML = svg; // Inject the rendered SVG
        
        // ... append the SVG, replace the original <pre><code> block ...
        preElement.parentNode?.replaceChild(container, preElement);
        codeElement.setAttribute("data-mermaid-rendered", "true"); // Mark as rendered

        // Use ReactDOM.createRoot to render React buttons (copy/fullscreen) directly into the new container
        import("react-dom/client").then(({ createRoot }) => {
          const root = createRoot(buttonContainer);
          root.render(
            <DiagramButtons
              onCopy={() => copyToClipboard(svgWrapper.querySelector("svg"), diagramId)}
              onFullscreen={() => setFullscreenDiagram({ element: svgWrapper, type: extractDiagramType(mermaidCode), id: diagramId })}
              copied={copiedStates.get(diagramId) || false}
              diagramType={extractDiagramType(mermaidCode)}
            />
          );
        });
      }
    ...
    renderDiagrams(); // Initial render on component mount

    // Use MutationObserver to detect if new Mermaid blocks appear later (e.g., lazy loaded content)
      ...
      mutations.forEach((mutation) => {
        mutation.addedNodes.forEach((node) => {
          if (node.nodeType === Node.ELEMENT_NODE) {
            ...
            if (element.querySelector?.("code.language-mermaid:not([data-mermaid-rendered])")) {
              hasNewMermaid = true;
            }
          }
        });
      });
      if (hasNewMermaid) { setTimeout(renderDiagrams, 100); } // Render new ones
    });
    observer.observe(document.body, { childList: true, subtree: true });
    return () => observer.disconnect(); // Cleanup
  }, [mermaidInitialized, extractDiagramType, copyToClipboard]);

  // ... (Other useEffects for ESC key, handling copy button state updates) ...

  return (
    // The FullscreenModal component is rendered here
    <FullscreenModal
      isOpen={!!fullscreenDiagram}
      onClose={() => setFullscreenDiagram(null)}
      // ... (props for copy, diagram type) ...
    >
      {/* Renders the SVG content inside the modal */}
    </FullscreenModal>
  );
});
