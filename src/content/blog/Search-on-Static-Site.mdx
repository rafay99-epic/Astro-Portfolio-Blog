...
```typescript
const results = fuzzy.filter(searchQuery, posts, { extract: (el: Post) => el.data.title });
...
```jsx
  ...
  // Dummy posts array for demo purposes
  const posts = [
    { title: 'First Post' },
    { title: 'Second Post' },
  ...
  const SearchComponent = () => {
    ...
    const handleSearch = (e) => {
      ...
      if (searchQuery) {
        const searchResults = fuzzy.filter(searchQuery, posts, { extract: (post) => post.title });
        setResults(searchResults.map(result => result.original));
      } else {
        setResults([]);
      }
    };
